  ...
  return (
    <>
      <button
        ...
        onClick={handleClick}
        disabled={loading}
        className="flex w-full items-center justify-center gap-3 rounded-full border border-coral-soft bg-white px-4 py-3 text-sm font-semibold text-ink shadow-sm transition hover:bg-coral-soft/20 disabled:opacity-50"
      >
        <Logo />
        <span>{loading ? 'Redirection…' : PROVIDER_LABELS[provider]}</span>
      </button>
      {error && (
