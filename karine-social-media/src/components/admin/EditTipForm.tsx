...
export function EditTipForm({ tip }: { tip: Tip }) {
  ...
  // Liste des slides existantes (mutables : on peut en retirer)
  const [keptSlides, setKeptSlides] = useState<string[]>(tip.slides);
  // Nouvelles slides ajoutées (previews)
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // === Phase 1 : PATCH texte + liste des slides conservées ===
      ...
      // === Phase 2 : upload incrémental des nouvelles slides ===
      ...
      for (const f of extraFiles) {
        ...
        const r = await fetch(`/api/admin/tips/${tip.id}/asset`, {
          method: 'POST',
          body: asset,
        });
        if (!r.ok) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl bg-white/85 p-6 shadow-sm">
      <Field label="Label" required>
        <input name="label" required defaultValue={tip.label} className="input" />
      </Field>

      <Field label="Tags (séparés par des virgules)">
        <input name="tags" defaultValue={tip.tags.join(', ')} className="input" />
      </Field>
      ...
      <Field label="Statut">
        <select name="status" defaultValue={tip.status} className="input">
