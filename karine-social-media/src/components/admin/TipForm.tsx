...
export function TipForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // === Phase 1 : POST texte + cover (1ʳᵉ slide) ===
      setProgress({ done: 0, total: 0, label: 'Création de l’astuce…' });
      ...
      const res1 = await fetch('/api/admin/tips', { method: 'POST', body: phase1 });
      const json1 = await res1.json().catch(() => ({}));
      if (!res1.ok) throw new Error(json1?.error || 'Erreur création astuce');
      const newSlug: string = json1.slug;

      // === Phase 2 : upload incrémental des slides additionnelles ===
      ...
      for (const f of extraFiles) {
        ...
        const r = await fetch(`/api/admin/tips/${newSlug}/asset`, {
          method: 'POST',
          body: fd,
        });
        if (!r.ok) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl bg-white/85 p-6 shadow-sm">
      <Field label="Label" required>
        <input
          name="label"
          required
          placeholder="ex. Garder ses herbes fraîches au congélateur"
          className="input"
        />
      </Field>

      <Field label="Tags (séparés par des virgules)">
        <input name="tags" placeholder="conservation, herbes, économie" className="input" />
      </Field>
