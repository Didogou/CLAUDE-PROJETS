...
export function Logo({
  ...
  compact?: boolean;
  /** Force le mode compact indépendamment du scroll. Utilisé sur les
   *  pages internes pour que la marque reste petite et laisse la place
   *  au titre de la page (option Hybride C, validée 2026-06-08). */
      ...
      <div className="flex max-w-full flex-nowrap items-baseline justify-center gap-x-1 sm:gap-x-1.5 lg:gap-x-3">
        ...
        <Leaf
          className={`-translate-y-0.5 -rotate-12 self-center text-sage transition-[width,height] duration-300 ease-out ${
            isCompact ? 'h-4 w-4' : 'h-5 w-5 sm:h-6 sm:w-6 lg:h-8 lg:w-8'
          }`}
          strokeWidth={2.5}
        />
        <span
