...
export default async function PetitsDejeunersPage() {
  const [recipes, userHasPlan] = await Promise.all([getRecipesByCategory('petit_dejeuner'), userHasPlanAccess()]);
  return (
    <div className="relative flex min-h-screen flex-col">
      <FloralBackground />
      <AppHeader />
      <main className="mx-auto w-full max-w-md flex-1 px-5 pb-8 lg:max-w-7xl lg:px-10">
        <CategoryListView category="petit_dejeuner" recipes={recipes} userHasPlan={userHasPlan} />
      </main>
      <BottomNav />
    </div>
  );
