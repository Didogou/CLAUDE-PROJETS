...
export default async function SaladesPage() {
  const [recipes, userHasPlan] = await Promise.all([getRecipesByCategory('salade'), userHasPlanAccess()]);
  return (
    <div className="relative flex min-h-screen flex-col">
      <FloralBackground variant="salade" />
      <AppHeader />
      <main className="mx-auto w-full max-w-md flex-1 px-5 pb-8 lg:max-w-7xl lg:px-10">
        <CategoryListView category="salade" recipes={recipes} userHasPlan={userHasPlan} />
      </main>
      <BottomNav />
    </div>
  );
