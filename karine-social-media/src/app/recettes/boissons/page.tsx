...
export default async function BoissonsPage() {
  const [recipes, userHasPlan] = await Promise.all([getRecipesByCategory('boisson'), userHasPlanAccess()]);
  return (
    <div className="relative flex min-h-screen flex-col">
      <FloralBackground />
      <AppHeader />
      <main className="mx-auto w-full max-w-md flex-1 px-5 pb-8 lg:max-w-7xl lg:px-10">
        <CategoryListView category="boisson" recipes={recipes} userHasPlan={userHasPlan} />
      </main>
      <BottomNav />
    </div>
  );
