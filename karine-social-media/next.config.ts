...
const nextConfig: NextConfig = {
  ...
  async headers() {
    ...
    const SECURITY_HEADERS = [
      ...
      // ⚠️ camera=(self) + microphone=(self) — autorise notre propre origine
      // UNIQUEMENT. microphone : cuisine guidée (Web Speech API). camera :
      // analyse photo repas in-app (getUserMedia, Phase 3).
      // Sans (self), le navigateur refuse getUserMedia/SpeechRecognition avec
      // "NotAllowedError" même quand l'utilisateur a cliqué "Autoriser" — le
      // header HTTP prime sur le réglage UI, et AUCUNE entrée caméra/micro
      // n'apparaît dans les réglages du site (politique = blocage total).
      // Bug micro 2026-06-13 (microphone=()) ; bug caméra 2026-06-14 (camera=()).
      {
