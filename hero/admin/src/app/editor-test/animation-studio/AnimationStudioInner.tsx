...
import { ArrowLeft, Sparkles, Sun, Moon, Eye } from 'lucide-react'
...
// AnimationStudioTimeline (legacy linéaire) remplacé par MultiTrackEditor
// (refonte 2026-05-12 — 4 pistes drag-drop style CapCut/Premiere).
import MultiTrackEditor from './components/multi-track-timeline/MultiTrackEditor'
import AnimationStudioPromptZone from './components/AnimationStudioPromptZone'
import AnimationStudioRail from './components/AnimationStudioRail'
import AnimationStudioBankPanel, { type BankAsset } from './components/AnimationStudioBankPanel'
import './components/animation-studio-bank.css'
import AnimationStudioCharactersDrawer from './components/AnimationStudioCharactersDrawer'
import AnimationStudioLightbox from './components/AnimationStudioLightbox'
import { type AiPaletteContext, type AiExtraction } from './components/AnimationStudioAiPalette'
import AnimationStudioAiChat from './components/AnimationStudioAiChat'
import type { ChatMessage, ChatShotProposal } from '@/lib/ai-chat-types'
import PelliculeExitEditor from './components/PelliculeExitEditor'
...
export default function AnimationStudioInner() {
  ...
  /** Refonte 2026-05-14av — État Play/Pause de la timeline propagé au Preview
   *  (sync lecture). Mis à jour via callback onPlayingChange du MultiTrackEditor. */
  ...
  return (
    ...
    <div className={`as-root ${theme === 'light' ? 'theme-light' : ''}`}>
      ...
      <div className={`as-body${drawer === 'images' ? ' has-bank' : ''}`}>
        ...
        >
          {/* Refonte 2026-05-12 : remplace l'ancienne AnimationStudioTimeline
           *  (liste linéaire de pellicules) par MultiTrackEditor (bibliothèque
           *  + 4 pistes drag-drop style CapCut). Phase 1bis : actions création
           *  pellicules réintégrées dans la toolbar library. Persistance des
           *  drops audio/texte = Phases 2/3. */}
          <MultiTrackEditor
            bookId={bookId}
            bankImages={sectionBankImages}
            visibleTracks={['video_image']}
            visibleSections={['animations', 'images']}
            onOpenEffects={(pelliculeId) => {
              const t = buildTargetFromPellicule(pelliculeId)
              if (t) setEffectsModalTarget(t)
            }}
            onOpenCapture={(pelliculeId) => {
              const t = buildTargetFromPellicule(pelliculeId)
              if (t) setCaptureModalTarget(t)
            }}
            /* Refonte 2026-05-16 — pas de bouton "Modifier" sur les pellicules
             *  dans Studio Animation : on est déjà dans le Studio. Le bouton
             *  Modifier sert uniquement à OUVRIR un Studio depuis ailleurs
             *  (Studio Section). */
            onCursorPelliculeChange={(pelliculeId) => {
              // Refonte 2026-05-14av : timeline cursor change → highlight
              // la pellicule + le preview seek dessus (déjà branché via
              // selectedPelliculeId prop sur AnimationStudioPreview).
              if (pelliculeId) setAnimationSelectedPellicule(pelliculeId)
            }}
            onPlayingChange={(playing) => {
              // Sync lecture timeline → preview. Le Preview écoute
              // externalPlaying via prop ci-dessous.
              setTimelinePlaying(playing)
            }}
            onSelectAnimation={(animationId) => {
              // Refonte 2026-05-14ay : click sur tile pellicule → highlight
              // sur la timeline + preview seek (déjà branché via selectedPelliculeId).
              setAnimationSelectedPellicule(animationId)
            }}
            onSelectImage={(imageId) => {
              // Click sur tile image → tente de trouver la pellicule image_static
              // qui utilise cette image comme firstFrameUrl, et la sélectionner.
              const m = imageId.match(/^section-image-(\d+)$/)
              if (!m) return
              // Retrouve l'URL de l'image dans bankImages → matche les pellicules
              const img = sectionBankImages.find(i => i.id === imageId)
              if (!img) return
              const pell = animationPellicules.find(p =>
                p.firstFrameUrl === img.url || p.firstFrameUrl === img.url + '?',
              )
              if (pell) setAnimationSelectedPellicule(pell.id)
            }}
            onRenameAnimation={(animationId, newLabel) => {
              // Refonte 2026-05-14ax — double-click label tile pellicule.
              // 1. Update local state (feedback immédiat).
              // 2. PATCH asset en DB (best-effort, rollback en cas d'échec).
              updateAnimationPellicule(animationId, { label: newLabel })
              if (!bookId) return
              void (async () => {
                try {
                  const r = await fetch(`/api/assets/animation/${animationId}?bookId=${bookId}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ label: newLabel }),
                  })
                  if (!r.ok) {
                    const errBody = await r.json().catch(() => ({})) as { error?: string }
                    console.warn('[AnimationStudio] rename PATCH failed:', errBody.error ?? r.status)
                  }
                } catch (err) {
                  console.warn('[AnimationStudio] rename PATCH error:', err)
                }
              })()
            }}
            onEditImage={(imageId) => {
              // B.2 : crayon sur tile Image → route vers Studio Designer.
              // ID format = "section-image-${planIdx}".
              const m = imageId.match(/^section-image-(\d+)$/)
              if (!m || !sectionIdFromUrl) return
              const params = new URLSearchParams({
                planIndex: m[1],
                sectionId: sectionIdFromUrl,
                returnSectionId: sectionIdFromUrl,
              })
              router.push(`/editor-test/new-layout?${params.toString()}`)
            }}
            onDeleteImage={(imageId) => {
              // B.2 : corbeille sur tile Image → delete entry + storage.
              const m = imageId.match(/^section-image-(\d+)$/)
              if (!m || !sectionIdFromUrl) return
              if (!confirm('Supprimer cette image ?\n\nCette action est irréversible (entrée DB + fichier storage supprimés).')) return
              const planIdx = parseInt(m[1], 10)
              const target = sectionBankImages.find(img => img.id === imageId)
              if (target?.url) {
                void fetch('/api/storage/delete', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ urls: [target.url] }),
                }).catch(err => console.warn('[AnimationStudio] image storage delete failed:', err))
              }
              // PATCH section.images filtré + reload UI
              void (async () => {
                try {
                  const sectionRes = await fetch(`/api/sections/${sectionIdFromUrl}`)
                  if (!sectionRes.ok) throw new Error(`section read HTTP ${sectionRes.status}`)
                  const { section } = await sectionRes.json() as { section: { images?: import('@/types').SectionImage[] } }
                  const newImages = (section.images ?? []).filter((_, i) => i !== planIdx)
                  const patchRes = await fetch(`/api/sections/${sectionIdFromUrl}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                  ...
                  setSectionBankImages(prev => prev.filter(img => img.id !== imageId))
                } catch (err) {
                  console.error('[AnimationStudio] delete image failed:', err)
                  alert(`Suppression image échouée : ${err instanceof Error ? err.message : String(err)}`)
                }
              })()
            }}
            onSelectBlock={(block) => {
              if (block.kind === 'video' || block.kind === 'image_static') {
                setAnimationSelectedPellicule(block.pelliculeId)
              }
            }}
            onDropFromLibrary={(track, payload, dropMs) => {
              // V1 image_static (refonte 2026-05-13) : drag depuis library
              // Images → crée une nouvelle pellicule animation utilisant cette
              // image comme `firstFrameUrl` (= source LTX I2V). L'auteur peut
              // ensuite éditer le prompt et générer la pellicule.
              if (payload.blockKind === 'image_static' && track === 'video_image') {
                const imageUrl = payload.data.imageUrl as string | undefined
                if (imageUrl) {
                  addAnimationPellicule({
                    firstFrameUrl: imageUrl,
                  })
              ...
              // Autres drops (sfx/music/text) : pas géré V1 dans cet écran
              // (= ils sont gérés au niveau Studio Section).
              console.log('[MultiTrackEditor] drop ignored:', { track, payload, dropMs })
            }}
            onAddPellicule={() => addAnimationPellicule()}
            onContinueVideo={handleContinueVideo}
            onUploadVideo={handleUploadVideo}
            onDeleteAnimation={(pelliculeId) => {
              // Refonte 2026-05-14 — Cascade complète : DELETE storage +
              // DELETE asset DB (RPC scoped via /api/assets/animation/[id]?bookId=X).
              // Le RPC supprime asset_usage + section_timeline + asset row si
              // plus aucune ref. Côté state : retire la pellicule.
              const p = animationPellicules.find(pp => pp.id === pelliculeId)
              if (p) {
                const urls = [p.videoUrl, p.firstFrameUrl, p.lastFrameUrl]
                ...
                if (urls.length > 0) {
                  ...
                  }).catch(err => console.warn('[AnimationStudio] storage delete failed:', err))
                }
              }
              autoCreatedPelliculeRef.current = true
              removeAnimationPellicule(pelliculeId)
              if (bookId) {
                void fetch(`/api/assets/animation/${pelliculeId}?bookId=${bookId}`, {
                  method: 'DELETE',
                }).catch(err => console.warn('[AnimationStudio] asset DELETE failed:', err))
              }
            }}
            onDeleteBlock={(block) => {
              // Refonte 2026-05-14bi — Croix sur bloc timeline = retire UNIQUEMENT
              // de la timeline (state local). L'asset reste dans la banque V2 et
              // les fichiers storage sont préservés pour réutilisation. Pour
              // supprimer définitivement (cascade DELETE), utiliser la corbeille
              // sur la tile de la banque (= onDeleteAnimation, scoped au livre).
              if (block.kind !== 'video' && block.kind !== 'image_static') return
              autoCreatedPelliculeRef.current = true
              removeAnimationPellicule(block.pelliculeId)
            }}
          />
